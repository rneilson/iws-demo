...
A local version of `settings.py` is created by `setup.py` in the `iws/iws/` directory. This file is not tracked by git, and thus can be customized as desired without conflicting with upstream changes. However, any such changes must be either manually merged, or `settings.py` can be deleted and regenerated by `setup.py`.

Please refer to the [Django documentation](https://docs.djangoproject.com/en/1.9/ref/settings/) for further details on available settings.

### Django admin

By default, the Django admin application is present but not enabled for security reasons. `setup.py` will ask if you wish to enable it during generation of `settings.py`; it can also be enabled later by appending the following to `settings.py`:
```
IWS_ENABLE_ADMIN = True
```

If enabled, it will be available under the path `/admin/`

It is recommended to restrict access; the provided Nginx configuration template includes an optional section to enable access controls for the `/admin/` path.
